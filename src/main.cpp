...
CBigNum bnProofOfStakeLimit(~uint256(0) >> 1000);//  PoS starting difficulty = 1.2205
...
unsigned int nStakeMinAge = 12 * 60 * 60; // half day
...
const string strMessageMagic = "BitAltcoinX Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // bitaltcoinx: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // bitaltcoinx: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// bitaltcoinx: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // bitaltcoinx: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // bitaltcoinx: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        // bitaltcoinx: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // bitaltcoinx: track money supply and mint amount info
...
// bitaltcoinx: total coin age spent in transaction, in the unit of coin-days.
...
// bitaltcoinx: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // bitaltcoinx: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // bitaltcoinx: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // bitaltcoinx: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // bitaltcoinx: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // bitaltcoinx: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // bitaltcoinx: check proof-of-stake
    ...
    // bitaltcoinx: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // bitaltcoinx: getblocks may not obtain the ancestor block rejected
    ...
    // bitaltcoinx: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BitAltcoinX", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x43915abceef29dc5025569a07f290c06708fdd51b459d392ae911c595ab282ec"));
        block.print();
        ...
        // bitaltcoinx: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // bitaltcoinx: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // bitaltcoinx: tell downloading node about the latest block if it's
